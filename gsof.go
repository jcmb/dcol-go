...
func expandedToFlatBuffer(recs []ExpandedRecord) []byte {
	...
	for _, e := range recs {
		...
		if len(e.Inner) > 255 {
			// GSOF length is one byte; preserve oversized bodies as consecutive
			// same-type chunks (e.g. future growth) instead of truncating.
			inner := e.Inner
			for len(inner) > 0 {
				n := len(inner)
				if n > 255 {
				...
				out = append(out, byte(e.MsgType), byte(n))
				out = append(out, inner[:n]...)
				inner = inner[n:]
			}
			continue
