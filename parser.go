...
func (p *Parser) noteUndecodedAfterSync(reason, remoteAddr string, env Env, discarded []byte) {
	n := len(discarded)
	if !p.synced || n <= 0 {
		return
	}
	hint := dcolDiscardedBytesHint(discarded)
	msg := fmt.Sprintf("[%s] WARNING: %d undecoded byte(s) after DCOL sync (%s) remote=%s%s",
		time.Now().Format("15:04:05"), n, reason, remoteAddr, hint)
	p.pendingStreamWarnings = append(p.pendingStreamWarnings, msg)
	env.logf(2, "[VERBOSE2] %s\n", msg)
}

// dcolDiscardedBytesHint explains common stray bytes seen when resynchronizing on STX (0x02).
func dcolDiscardedBytesHint(discarded []byte) string {
	for _, b := range discarded {
		if b == 0x15 {
			return " — discarded data includes 0x15 (Trimble DCOL NAK packet type); possible truncated/lost STX before a full NAK frame"
		}
	}
	return ""
}
	...
	for len(p.buf) >= 6 {
		...
		if stxIdx == -1 {
			if len(p.buf) > maxDCOLBufWithoutSTX {
				...
				p.noteUndecodedAfterSync("discarded without STX (buffer cap)", env.RemoteAddr, env, discarded)
				p.buf = nil
			}
			return
		}
		if stxIdx > 0 {
			discarded := append([]byte(nil), p.buf[:stxIdx]...)
			p.noteUndecodedAfterSync("leading bytes before STX", env.RemoteAddr, env, discarded)
			p.buf = p.buf[stxIdx:]
		}
		if len(p.buf) < 6 {
			return
		...
		if p.buf[totalExpectedLen-1] != 0x03 { // ETX
			p.noteUndecodedAfterSync("invalid ETX (slipped 1 byte)", env.RemoteAddr, env, p.buf[:1])
			p.buf = p.buf[1:]
			continue
		}
		...
		if csum != p.buf[totalExpectedLen-2] {
			...
			p.noteUndecodedAfterSync("checksum mismatch (slipped 1 byte)", env.RemoteAddr, env, p.buf[:1])
			p.buf = p.buf[1:]
			continue
		}
