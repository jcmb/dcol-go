...
func TestDCOL_UndecodedAfterSyncWarning(t *testing.T) {
	...
	if strings.Contains(ev.StreamWarnings[0], "NAK") {
		t.Fatalf("unexpected NAK hint for non-0x15 prefix: %q", ev.StreamWarnings[0])
	}
...
func TestDCOL_UndecodedLeadingBeforeSTX_IncludesNAKHint(t *testing.T) {
	p := newTestParser()
	var got []Message
	emit := func(m Message) { got = append(got, m) }
	env := Env{Verbose: 0, RemoteAddr: "tcp:1", TransportIsUDP: false}
	...
	frame2 := []byte{0x02, 0x00, 0x40, 0x05, 0x08, 0x00, 0x00, 0x01, 0x00, 0x4E, 0x03}
	p.Process(frame1, env, emit)
	p.Process(append([]byte{0x15}, frame2...), env, emit)
	if len(got) != 2 {
		t.Fatalf("want 2 messages, got %d", len(got))
	}
	ev := got[1]
	...
	w := ev.StreamWarnings[0]
	if !strings.Contains(w, "0x15") || !strings.Contains(w, "NAK") {
		t.Fatalf("expected NAK hint in warning: %q", w)
	}
	if ev.PacketType != 0x40 {
		t.Fatalf("packet type want 0x40 got %#x", ev.PacketType)
	}
